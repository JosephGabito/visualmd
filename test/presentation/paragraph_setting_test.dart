...
void main() {
  ...
  testWidgets('long nested marks keep one prose flow and accessible text', (
    tester,
  ) async {
    final semantics = tester.ensureSemantics();
    const source =
        'This combined statement carries important words in another voice '
        'through enough ordinary prose to wrap over several rendered lines '
        'without exposing notation, splitting accessibility, or changing the '
        'baseline rhythm of the paragraph that contains it.';
    final block = const MarkdownDocumentParser()
        .parse('***$source***')
        .blocks
        .single;
  ...
  testWidgets(
    'long strikethrough reflows on the prose grid and stays accessible',
    (tester) async {
      final semantics = tester.ensureSemantics();
      const source =
          'This superseded explanation remains readable long enough to wrap '
          'over several rendered lines without changing the measure, leading, '
          'selection, accessibility, or baseline rhythm of its paragraph.';
      final block = const MarkdownDocumentParser()
          .parse('~~$source~~')
          .blocks
          .single;
