...
(`lib/api/widgets/panel_resize_handle.dart:8-35`).
...
(`lib/api/widgets/panel_resize_handle.dart:12-21`). Its resting rule is the
ordinary panel border. Hover or focus thickens it to 2 px and changes it to the
theme accent over 100 ms (`lib/api/widgets/panel_resize_handle.dart:74-77`,
`lib/api/widgets/panel_resize_handle.dart:111-119`). The mouse cursor becomes a
column-resize cursor (`lib/api/widgets/panel_resize_handle.dart:97-100`).
...
narrower (`lib/api/widgets/panel_resize_handle.dart:48-53`).
...
(`lib/api/widgets/panel_resize_handle.dart:55-72`); pointer focus, dragging and
double-click are wired together at `:101-110`. `DragStartBehavior.down` keeps
...
`onCommit` and `onReset` callbacks (`lib/api/widgets/panel_resize_handle.dart:16-31`).
...
(`lib/api/widgets/panel_resize_handle.dart:101-108`).

## Lifecycle

The widget keeps only hover and focus state. Its private `FocusNode` is disposed
with the handle (`lib/api/widgets/panel_resize_handle.dart:37-46`). The handle
is mounted only in the wide shell; compact overlays retain their divider and
have no resizing interaction (`lib/api/screens/reader_screen.dart:398-410`,
`lib/api/screens/reader_screen.dart:425-438`).
...
(`lib/api/widgets/panel_resize_handle.dart:78-92`).
