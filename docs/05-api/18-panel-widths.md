...
(`lib/api/layout/panel_widths.dart`).
...
(`lib/api/layout/panel_widths.dart`). These are chrome bounds: enough room
...
(`lib/api/layout/panel_widths.dart`,
`lib/api/layout/panel_widths.dart`). Drag updates pass through
...
(`lib/api/layout/panel_widths.dart`). The reading column therefore wins
...
leaving the underlying room visible (`lib/api/layout/panel_widths.dart`,
`lib/api/layout/panel_widths.dart`).
...
preferences before constructing the controller (`lib/main.dart`).
...
existing proportion rather than overflowing (`lib/api/layout/panel_widths.dart`).
...
compact clamping (`test/presentation/panel_widths_test.dart`).
