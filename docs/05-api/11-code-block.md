...
(`lib/api/widgets/code_block.dart:5-23`). It exists because prose and code want
...
(`lib/api/render/document_view.dart:238-258`), which supplies the mono style
...
(`lib/api/render/document_view.dart:72-97`): code is given about a third more
...
line (`lib/api/render/document_view.dart:136-150`).

**It sits on the beat, and it has no border.** Each line of code is set to
exactly one beat (`lib/api/render/reading_theme.dart:137-146`,
`lib/api/render/reading_theme.dart:259-262`) and the padding
is half a beat above and below
(`lib/api/render/document_view.dart:207-213`), so the whole block comes to a
...
to carry is gone (`lib/api/render/document_view.dart:214-219`): the block's own
ground already says what it is, so a border was a second signal, and it had a
height that broke the grid.

The block itself (`lib/api/widgets/code_block.dart:35-64`):
...
- A `Container` with the decoration it was given, clipped to its rounded
  ...
  (`lib/api/widgets/code_block.dart:38-39`).
- A `Scrollbar` over a horizontal `SingleChildScrollView`
  (`lib/api/widgets/code_block.dart:40-51`). `thumbVisibility` is true because
...
(`lib/infrastructure/markdown/markdown_document_parser.dart:152-155`).

Selection is left to the reading pane's `SelectionArea`
(`lib/api/widgets/reading_pane.dart:129`); the text carries only its own
selection colour so a highlighted line still reads against the block's
background (`lib/api/widgets/code_block.dart:52-60`). Nesting a second
...
cell (`lib/api/render/inline_composer.dart:59-60`,
`lib/api/render/reading_theme.dart:193-212`).
...
| `padding` | `EdgeInsets` | Half a beat vertically (`lib/api/render/document_view.dart:207-213`) |
| `decoration` | `Decoration` | `codeBackground` and an 8 px radius, no border (`lib/api/render/document_view.dart:214-219`) |
...
(`lib/api/widgets/code_block.dart:26-32`). Blocks live as long as the document
...
- A block shorter than the column simply does not scroll — there is nothing to
  ...
  (`test/presentation/code_block_test.dart:82-88`).
...
Guarded by `test/presentation/code_block_test.dart:55-88`: a long line can be
...
`test/presentation/document_view_test.dart:95-165`.

## Transition

Code now runs wider than prose, but only to 1.35 × the measure
(`lib/api/render/reading_theme.dart:221-224`). On a large screen there is room to
