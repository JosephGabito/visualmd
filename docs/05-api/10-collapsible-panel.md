...
`visible` changes (`lib/api/widgets/collapsible_panel.dart`), over 180 ms
...
| What | How | Where |
|---|---|---|
| The space it takes | `Align(widthFactor: t)` inside a `ClipRect`, so the row gives the width back to the page as it shrinks | `lib/api/widgets/collapsible_panel.dart` |
| Which way it goes | The `Align` is pinned to the edge the panel lives against — `centerRight` for a left panel, `centerLeft` for a right one — so the *far* side is what gets clipped and it reads as sliding out rather than squashing | `lib/api/widgets/collapsible_panel.dart` |
| How it dims | Opacity eased from the same value, so it is on its way out before it fades | `lib/api/widgets/collapsible_panel.dart` |

The panel keeps its full width the entire time, inside a `SizedBox`
(`lib/api/widgets/collapsible_panel.dart`). This is the part that matters:
...
(`lib/api/widgets/collapsible_panel.dart`), so a hidden panel costs nothing.

The `child` is passed through the builder rather than rebuilt inside it
(`lib/api/widgets/collapsible_panel.dart`), so the panel's contents are
...
| `side` | `PanelSide` | `left` or `right` — the edge it is anchored to (`lib/api/widgets/collapsible_panel.dart`) |
...
- A reader who has asked for less motion gets the change at once, with no
  animation (`lib/api/widgets/collapsible_panel.dart`,
  `lib/api/widgets/collapsible_panel.dart`).
...
- The panel's contents keep their layout while leaving, which
  `test/presentation/reader_chrome_test.dart` guards by measuring the
