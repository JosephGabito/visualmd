...
`LibraryPalette` (`lib/api/theme/library_theme.dart`) is the nine tokens
as the widget tree reads them, plus the `copyWith` and `lerp` pair Flutter
needs to animate a theme change (`lib/api/theme/library_theme.dart`).
It is built from a `ThemePalette` by `LibraryPalette.of`
(`lib/api/theme/library_theme.dart`) and read anywhere as
`context.palette` (`lib/api/theme/library_theme.dart`).
...
`LibraryTypefaces` (`lib/api/theme/library_theme.dart`) holds the
theme's three family names and hands out text styles:

| Voice | Signature | Citation |
|-------|-----------|----------|
| `serif` | colour, size, height, weight, style | `lib/api/theme/library_theme.dart` |
| `sans` | colour, size, height, weight | `lib/api/theme/library_theme.dart` |
| `mono` | colour, size, height | `lib/api/theme/library_theme.dart` |
...
four OFL files ship as assets (`pubspec.yaml`) and
...
producing it in that face (`lib/api/theme/library_theme.dart`) — the
...
Literata 7–72, Inter 14–32 (`lib/api/theme/font_licences.dart`) — and
`_font` hands the axis the size actually being drawn, clamped to that range
(`lib/api/theme/library_theme.dart`). It is the difference between a
font that has been scaled and one that has been designed.

Two facts were measured before relying on it
(`test/typography_measure_test.dart`): `TextStyle.fontWeight` still
...
`libraryTheme(ReaderTheme)` (`lib/api/theme/library_theme.dart`) is the
...
(`lib/api/theme/library_theme.dart`) and tints the few Material surfaces the
app shows from the same tokens (`lib/api/theme/library_theme.dart`):
...
(`lib/api/theme/library_theme.dart`), so no Material default leaks a
...
(`lib/api/render/reading_theme.dart`). The division of labour:
