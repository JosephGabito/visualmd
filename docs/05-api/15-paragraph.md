...
a strut and an indent (`lib/api/render/document_view.dart:618-639`). Building
it happens in three steps (`lib/api/render/document_view.dart:641-678`):

1. **Split the mark.** `splitHangingMark` takes an opening mark off the front
   of the spans if there is one (`lib/api/render/document_view.dart:654-678`).
2. **Bind the widow.** `bindWidow` counts the whole paragraph, then descends
   ...
   (`lib/api/render/document_view.dart:582-652`).
...
(`lib/api/render/document_view.dart:188`); see
...
(`lib/api/render/document_view.dart:668-678`). Two consequences of doing it
this way:
...
(`lib/api/render/document_view.dart:665-667`). The measurement receives the
...
hangs off the line (`lib/api/render/document_view.dart:628-630`).

### What deliberately does not hang or bind

- A paragraph opening with a link or an emphasised run
  (`lib/api/render/document_view.dart:652-660`). Pulling those into the margin
  would move meaning rather than ink.
- A paragraph ending in code or a link
  (`lib/api/render/document_view.dart:607-616`). It ends with something the
...
- A paragraph shorter than `WidowBinding.leastWords`, counted across the whole
  ...
  (`lib/api/render/document_view.dart:588-596`).
...
- Empty spans, a first span that is not text, or an empty first run all return
  the paragraph unchanged (`lib/api/render/document_view.dart:659-664`).
...
`test/presentation/paragraph_setting_test.dart:217-273` covers both halves:
...
- **The indent is one rendered em**
  (`lib/api/render/reading_theme.dart:264-268`). A reader who wanted a deeper
