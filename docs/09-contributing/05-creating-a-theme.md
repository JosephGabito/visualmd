...
(`lib/api/widgets/theme_picker.dart:51-54`). The folder is created on first
launch, with a `README.md` describing this same format written into it
(`lib/infrastructure/io/reader_files.dart:19-25`, `lib/infrastructure/io/reader_files.dart:71`).

Every `.json` file in that folder is read, in name order; anything else is
ignored (`lib/infrastructure/io/reader_files.dart:54-67`). Themes are loaded
...
|-------|----------|---------|
| `id` | yes | Lowercase letters, digits and hyphens, starting with a letter or digit (`lib/presentation/theme/reader_theme.dart:44-47`). |
| `name` | yes | What the menu shows. |
| `brightness` | yes | `light` or `dark` (`lib/presentation/theme/reader_theme.dart:48-53`). |
...
| `typefaces.serif` / `.sans` / `.mono` | no | Bundled or Google Fonts family names. Each falls back independently (`lib/presentation/theme/theme_typefaces.dart:16-29`). |

`schema` is not read today; write `1` so a future loader can tell what it is
looking at (`lib/presentation/theme/reader_theme.dart:11`).

Colours accept `#rgb`, `#rrggbb` and `#rrggbbaa`, with or without the `#`
(`lib/presentation/theme/theme_palette.dart:103-114`). Three-digit hexes expand the way
CSS does, and a missing alpha means opaque.

## Two things worth knowing

**An `id` that matches a built-in replaces it**
(`lib/presentation/theme/theme_registry.dart:32-38`). To warm up Paper by a few
...
(`lib/presentation/theme/theme_registry.dart:54-58`). The reason is printed at startup
(`lib/main.dart:40-45`) and the menu shows how many files were skipped
(`lib/api/widgets/theme_picker.dart:109-113`). Every other theme still loads.
...
`test/presentation/theme_test.dart:47-65`, which is also a compact reference
