...
| Notarized direct macOS download | Packaging | none | The Mac App Store archive is team-signed and validated. A download distributed outside the store still needs a Developer ID Application identity, notarization, stapling, and clean-machine verification. |
...
| Item | Landed as |
|------|-----------|
| Windows build verification | The x64 release builds with Flutter 3.47.1 and Visual Studio 2022 on Windows 11, launches under Windows on ARM emulation, retains its measured typography, and reads a native folder recursively. Windows CI catches host-specific failures, builds the Microsoft Store MSIX with a non-production identity, and audits the complete payload without publishing it (`.github/workflows/validate.yml`, `bin/tools/validate-windows-bundle.ps1`, `bin/tools/validate-windows-store-package.ps1`). |
