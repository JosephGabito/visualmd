...
| Bundle fonts as assets | Alegreya, Literata, Inter and JetBrains Mono ship inside the app as variable fonts with their OFL notices (`pubspec.yaml`), registered for the licence page (`lib/api/theme/font_licences.dart`). `GoogleFonts` is now only a fallback for a family a theme names that we do not ship (`lib/api/theme/library_theme.dart`). Text draws with no network and no flash of a fallback face — and the metrics became measurable, which is what everything after it was built on. |
