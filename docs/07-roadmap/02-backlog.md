...
| Full-document select all on a lazy page | Kernel fix | api | The sliver keeps dragging and ordinary copying viewport-bounded, but Flutter's stock select-all action reaches only registered selectables. A complete Command-A contract must select and copy from `DocumentContent` without remounting every block or losing authored separators (`lib/api/render/document_view.dart`, `lib/api/widgets/reading_pane.dart`). |
