...
| Item | Landed as |
|------|-----------|
| Windows build verification | The x64 release builds with Flutter 3.47.1 and Visual Studio 2022 on Windows 11, launches under Windows on ARM emulation, retains its measured typography, and reads a native folder recursively. Windows CI now catches host-specific failures, while the tagged release workflow packages the complete bundle and refuses public release without Authenticode (`.github/workflows/validate.yml`, `.github/workflows/release-windows.yml`, `bin/tools/validate-windows-bundle.ps1`). |
