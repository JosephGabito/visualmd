...
1. **Markdown only.** A file whose name lacks one of the recognised
   extensions is skipped (`lib/domain/library/library_builder.dart`).
   The extension set is `.md`, `.markdown`, `.mdown`, `.mkd`, compared
   case-insensitively (`lib/domain/library/markdown_file.dart`).
2. **Hidden folders.** A path with any *folder* segment that starts with `.`
   ...
   `lib/domain/library/hidden_folders.dart`). The fixed names cover Node,
...
5. **Empty folders are pruned.** When converting, any sub-folder with a
   ...
   (`lib/domain/library/library_builder.dart`). Because pruning happens
...
6. **Natural order.** Folders sort by name and documents by file name using
   ...
   numbers (`lib/domain/library/natural_order.dart`), so `2-setup`
   precedes `10-deploy`.
...
`test/domain/library_builder_test.dart`, `test/domain/library_builder_test.dart`, `test/domain/library_builder_test.dart`, `test/domain/library_builder_test.dart`
and `test/domain/library_builder_test.dart`; row five is covered by `test/domain/library_builder_test.dart`.
...
- Unwanted file types and hidden paths are skipped rather than rejected. A
  ...
  constructed (`lib/domain/library/library_builder.dart`).
- An input with no markdown yields an empty library rather than an error; the
  UI shows an empty state (`test/domain/library_builder_test.dart`).

## Transition

- Adding a file type (`.mdx`, `.txt`) is one entry in
  `lib/domain/library/markdown_file.dart`; every adapter and the builder
  follow automatically.
- Hidden-folder names are a fixed set today
  (`lib/domain/library/hidden_folders.dart`). A user-editable ignore list
